...
def _run_target_setup(
    ...
    for setup_name in _setup_names_for_target(cfg, target):
        ...
        try:
            ...
            logger.warning("対象サーバ「%s」: 事前処理「%s」の実行に失敗しました: %s", target.name, setup_name, e)
        ...
        if exit_status != 0:
            logger.warning(
                "対象サーバ「%s」: 事前処理「%s」が失敗しました（exit=%s）", target.name, setup_name, exit_status
            )
            return (
                False,
...
def _run_target_checks(cfg: Config, target: Target, target_conn: SSHConnection) -> list[dict]:
    ...
    for check_name in _check_names_for_target(cfg, target):
        ...
        try:
            ...
            if exit_status != 0:
                # 非ゼロ終了コード自体はアプリとしての失敗ではなく正常応答として扱うため、
                # 調査の手掛かりとして残す程度のINFOレベルに留める（WARNINGにはしない）
                logger.info(
                    "対象サーバ「%s」: チェック「%s」がexit=%sで終了しました", target.name, check_name, exit_status
                )
            checks.append(
                {
                    "name": check_name,
            ...
            logger.warning("対象サーバ「%s」: チェック「%s」の実行に失敗しました: %s", target.name, check_name, e)
            checks.append(
                {
                    "name": check_name,
                    "command": check_def.command,
...
def _run_target(
    ...
    with ExitStack() as stack:
        ...
        if target_conn is None:
            ...
            logger.warning(
                "対象サーバ「%s」: 接続に失敗しました（ホップ「%s」）: %s", target.name, failed_hop, message
            )
            return {
                "name": target.name,
                "host": target.host,
...
def _target_has_problem(target_result: dict) -> bool:
    """対象サーバの結果に、ステージ失敗またはチェック失敗（exit!=0またはerror）が含まれるか判定する。

    実行完了時のサマリーログ（どの対象サーバに異常があったか）を組み立てるためのもの。
    """
    if not all(s["ok"] for s in target_result["stages"].values()):
        return True
    return any(c["error"] is not None or c["exit_status"] != 0 for c in target_result["checks"])
...
def run_checks(cfg: Config, secrets: Secrets, env_name: str | None = None) -> dict:
    ...
    try:
        ...
        message = _classify_connection_error(e)
        logger.error("環境「%s」: 踏み台へのネットワーク到達に失敗しました: %s", env.name, message)
        return {
            "environment": env.name,
            "stages": {
                "bastion_network": _stage(False, message),
        ...
        message = _classify_connection_error(e)
        logger.error("環境「%s」: 踏み台のSSH認証に失敗しました: %s", env.name, message)
        return {
            "environment": env.name,
            "stages": {
                "bastion_network": _stage(True),
                "bastion_auth": _stage(False, message),
            },
            "targets": [],
        }
    ...
    failed_targets = [t["name"] for t in target_results if _target_has_problem(t)]
    if failed_targets:
        logger.warning(
            "環境「%s」のチェックが完了しました（%.1f秒、異常あり: %s）",
            env.name,
            elapsed,
            ", ".join(failed_targets),
        )
    else:
        logger.info("環境「%s」のチェックが完了しました（%.1f秒、全て正常）", env.name, elapsed)
