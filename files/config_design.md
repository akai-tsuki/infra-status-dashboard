...
# ---------------------------------------------------
web:
  listen_addr: ":18080"  # 8080番はすでに使用されている環境が多いため18080番を既定値にしている（実環境での動作確認で判明）
  auth: none
```
...
## ロールごとの事前処理（role_setup）

`oc login`のように、チェックコマンドを実行する前に済ませておく必要がある
処理がある（Issue #7）。`role_setup`でロールごとに事前処理名のリストを定義し、
`setup_definitions`で実際のコマンドを定義する。
...
setup_definitions:
  oc_login:
    command: 'read -r OC_TOKEN && oc login --token="$OC_TOKEN" --server=https://api.example.local:6443 --insecure-skip-tls-verify'
    secret_key: oc_token   # secrets.yaml側のtargets[].setup_secretsのキー名
```

対象サーバのrolesに含まれる全ロールの事前処理を重複除去した上で、チェック実行前に
順に実行する。いずれかが失敗した場合は後続の事前処理・チェックは実行せず、
`target_setup`ステージを失敗として返す。

`secret_key`を指定した事前処理は、対応する値を**コマンド文字列に埋め込まず、
標準入力経由で渡す**。`oc login`のトークンのような値をコマンド文字列に直接
書くと、対象サーバ上の`ps`やログ、API結果に露出する可能性があるため。実際の
値は`config.yaml`ではなく`secrets.yaml`側の`targets[].setup_secrets`に書く。

`oc login`の結果（`~/.kube/config`）はディスク上に永続化されるため、事前処理と
チェックコマンドが別々のSSHセッション（`exec_command`呼び出し）であっても
状態は引き継がれる。セッションをまたいだ環境変数の共有のような複雑な仕組みは
不要。
